    ...
    <div class="decor1">
      <div class="form-inner1">
        <table class="table table-striped">
          ...
          <tbody>
          <tr v-for="(item, index) in studches.filter(function (studch){
            return studch.studId === user[0].id
          })" :key="index">
            <td>{{index + 1}}</td>
            <td>{{ item.ch.chName }}</td>
            <td>{{ item.ch.chScore }}</td>
            <td>{{ item.studChScore }}</td>
...
export default {
  ...
  created() {
    this.studchs = this.$store.state.studches.find(item => item.studId === user[0].id);
  },
  computed: {
    ...mapState({
