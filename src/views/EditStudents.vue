<template>
  <div class="decor1">
    <div class="form-inner1">
      ...
      <button @click="reg()" id="OK">OK</button>
...
export default {
  ...
  created() {
    if (this.id !== 0 && this.student) {
      ...
      this.pass = this.student.pass;
      this.totalScore = this.student.totalScore;
  ...
  methods:{
    async reg(){
      ...
      await this.$router.push({name: "LoMaSt"});
    },
    ...mapActions([
      'addStudent',
      'editStudent',
  ...
  computed: {
    ...
    student() {
      return this.id !== 0 ?
          this.$store.state.students.find(item => item.id === this.id):
          null;
    }
  },
  name: "EditStudents"
