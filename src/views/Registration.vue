<template>
  <div class="decor1">
    <div class="form-inner1">
      ...
      <input id="inputLogin" placeholder="Login" type="text" v-model="login">

      <label for="inputPass">Password</label>
      <input id="inputPass" placeholder="Password" type="text" v-model="pass">

      <label for="inputMaster">Class</label>
      <select class="form-control" id="inputMaster" v-model="classId">
        <option v-for="(source, key) in classes" :value="source.id">{{ source.classNum }}</option>
      </select>
...
export default {
  ...
  methods:{
    async reg(){
      ...
      if(this.$store.state.students.find(item => item.login === this.login) != null){
        alert("Пользователь с таким логином уже существует");
      }else {
        await this.addStudent(item);
        await this.$router.push({name: "LoHoStu"});
      }
    },
    ...mapActions([
      'addStudent',
